...
description: Design spec for the generated A4 resume and cover letter. Typed data owns the copy, CSS owns presentation, and the renderer emits self-contained HTML, JSON, and PDF-ready documents.
colors:
  surface: "#ffffff"
  ink: "#23343c"
  ink-strong: "#1f3139"
...
A single-person CV rendered as two A4 sheets from strongly typed source modules. The voice is
...
preview; the generated HTML, JSON, and PDF are artefacts, and every layout
decision is bound to A4 geometry rather than to a viewport.

Reference output: `tmp/resume.html` (screen) and `tmp/resume.pdf` (2 sheets).
Current fit: sheet 1 = 886.1px and sheet 2 = 890.4px against a 1122.5px sheet, so
both carry more than 230px of headroom. Re-measure after adding content; a taller
sheet becomes an extra PDF page rather than clipping.

## Colors

Neutral-dominant with a single desaturated accent. The active palette is
`slate-green`. Each approved alternative supplies an accent plus a matching soft
tint and hairline; applying one requires changing the active values in
`src/styles/base.css`.
...
- **Chip Ink (#2f4443):** left-column language and interests labels.
...
- **Accent Soft (#eef4f3) / Border Soft (#d9e4e2):** reserved soft accent and
  every hairline (entry separators and the skill-column divider).
...
- **UI:** Aptos / Segoe UI / Noto Sans / Liberation Sans / Arial. Everything else,
  from the 15.4px role label down to the 8.3px skill text.
...
- **Body copy runs at 10.7px / 1.28**, entry summaries at 10.3px, the technology
  ...
  in print and at zoom. Nothing in the document is smaller than 8.3px, and the
...
- **Sheet 2 is two columns too, as blocks:** earlier experience and projects each
  render as a two-column grid (32px column gap, 20px row gap, section title
...
The renderer places a work entry from its `page` field: `main` selects sheet 1
and `more` selects sheet 2. Four roles sit on each sheet. Sheets do not reflow
into each other, so rebalance the data after a content change when needed.
...
Square throughout: sheets, sections, and hairlines have no corner radius. The
44px profile photo is circular. Technology lists are plain 8.6px text separated
with middle dots.
...
- **Project entry** — title line as above, 10.3px description in `body-alt`, and
  an 8.6px technology line beneath.
- **Education entry** — institution 12.4px, qualification 10.3px in `body-alt`,
  dates at 9.3px, and grade at 8.6px.
- **Award entry** — title at 12.4px, awarder at 10.3px, date at 9.3px, and
  summary at 8.6px.
...
- Do re-measure both sheets after any content or type change with `just check`.
- Do move history between sheets with the work entry's `page` field rather than
  deleting content.
...
- Don't render text below 8.3px, and don't shrink information-carrying text below
...
`src/data/` is the copy source and is checked against the document contracts in
`src/data/types.ts`. `src/styles/base.css` owns shared tokens;
`src/styles/resume.css` and `src/styles/cover-letter.css` own document-specific
layout. `scripts/render-documents.ts` escapes data values, embeds the profile
photo, and writes self-contained HTML plus portable JSON to `tmp/`.

The cover letter uses the same 208mm sheet, colours, nameplate, and body size. It
differs in three deliberate ways: its contact block sits in the header at 9.3px
rather than in the rail at 8.7px, its keyline opens the letter with a 12px/20px
margin instead of the 4px/5px used between sections, and its line-height is 1.62
rather than 1.28 because a letter is read as prose.

`just render` creates the HTML and JSON. PDF export then takes a generated local
file or a served URL (`just pdf-file`, or `PDF_INPUT` / `PDF_OUTPUT`). Both routes
yield identical PDF text; the generated local file is the default.
